...
import {cellToCol, cellToRow} from './grid.js';
...
export function play(db, cb) {
  ...
  for (let cell = 0; cell < len; cell += 1) {
    const col = cellToCol(gridWidth, cell);
    const row = cellToRow(gridWidth, cell);
    // eslint-disable-next-line no-magic-numbers
    grid[cell] = ctx.getImageData(col, row, 1, 1).data[0] === 255 ? Cell.FLOOR : Cell.WALL;
  }
