...
export function play(db, cb) {
  const assets = [...db.assets.values()].reduce((acc, it) => {
    acc[it.key] = it.result;
    return acc;
  }, {});
  ...
  mazeImage.src = URL.createObjectURL(assets['maze.png']);

  const maze = new createjs.Bitmap(mazeImage);
  ...
  zombieImage.src = URL.createObjectURL(assets['zombie.png']);

  const zombie = new createjs.Bitmap(zombieImage);
  zombie.x = 1 * CELL_DIM;
  zombie.y = 9 * CELL_DIM;
  npc.push(zombie);
