...
export function play(db, cb) {
  ...
  zombie.sprite.x = 1 * CELL_DIM;
  zombie.sprite.y = 9 * CELL_DIM;
  ...
  zombie2.sprite.x = 24 * CELL_DIM;
  zombie2.sprite.y = 1 * CELL_DIM;
  ...
  const stageW = gridWidth * CELL_DIM; // 1280
  const stageH = gridHeight * CELL_DIM; // 1024;

  const viewMaxX = (stageW / SCALE) - (gridWidth * maze.scaleX);
  const viewMaxY = (stageH / SCALE) - (gridHeight * maze.scaleY);
