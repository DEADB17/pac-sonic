...
function handleTick() {
  ...
  let sx;
  const dx = (db.mainCanvas.width / (2 * SCALE)) - db.npc[1].x;
  if (dx > 0) sx = 0;
  else if (dx < db.viewMaxX) sx = db.viewMaxX * SCALE;
  else sx = dx * SCALE;
  db.stage.x = sx;

  let sy;
  const dy = (db.mainCanvas.height / (2 * SCALE)) - db.npc[1].y;
  if (dy > 0) sy = 0;
  else if (dy < db.viewMaxY) sy = db.viewMaxY * SCALE;
  else sy = dy * SCALE;
  db.stage.y = sy;
